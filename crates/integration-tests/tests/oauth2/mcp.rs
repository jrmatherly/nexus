...
#[tokio::test]
async fn access_denied_insufficient_scope() {
    ...
    let unique_client_id = format!("insufficient-scope-client-{}", chrono::Utc::now().timestamp());
    let client_response = hydra.create_client(&unique_client_id, "user:read").await.unwrap();
    let token_response = hydra
        .get_token(&client_response.client_id, &client_response.client_secret, "user:read")
        .await
        .unwrap();
    ...
    let status = response.status();
    assert_ne!(status, 401, "Should not be unauthorized with valid token");

    assert!(
        status == 403 || status == 200,
        "Should either be forbidden or allowed based on scope enforcement"
    );
}

#[tokio::test]
async fn access_allowed_sufficient_scope() {
    let config = super::oauth_config_complex_scopes();
    let server = TestServer::builder().build(config).await;
    ...
    let unique_client_id = format!("sufficient-scope-client-{}", chrono::Utc::now().timestamp());
    let client_response = hydra.create_client(&unique_client_id, "mcp:execute").await.unwrap();
    ...
    assert_ne!(response.status(), 403, "Should allow access with sufficient scope");
    assert_ne!(response.status(), 401, "Should not be unauthorized with valid token");
}

#[tokio::test]
async fn access_no_scope_config_allows_any() {
    ...
    let unique_client_id = format!("any-scope-client-{}", chrono::Utc::now().timestamp());
    let client_response = hydra.create_client(&unique_client_id, "custom:scope").await.unwrap();
    ...
    assert_ne!(response.status(), 403, "Should allow access when no scopes configured");
    assert_ne!(response.status(), 401, "Should not be unauthorized with valid token");
}
