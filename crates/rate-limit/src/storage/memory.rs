...
mod tests {
    ...
    async fn test_rate_limit_allows_under_limit() {
        ...
        // Test with 10 requests per minute
        for i in 0..10 {
            let result = storage
                .check_and_consume(&format!("test-{i}"), 10, Duration::from_secs(60))
                .await
                .unwrap();
    ...
    async fn test_rate_limit_blocks_over_limit() {
        ...
        // Test with 2 requests per minute, try 3
        for _ in 0..2 {
            let result = storage
                .check_and_consume("test-key", 2, Duration::from_secs(60))
                .await
                .unwrap();
            assert!(result.allowed);
        }
    ...
    async fn test_different_keys_have_separate_limits() {
        ...
        for _ in 0..2 {
            let result = storage
                .check_and_consume("key1", 2, Duration::from_secs(60))
                .await
                .unwrap();
            assert!(result.allowed);
        }
        ...
        let result = storage
            .check_and_consume("key2", 2, Duration::from_secs(60))
            .await
            .unwrap();
        assert!(result.allowed);
    }
